...
## 여기에 뭘 올리는가 (2026-08-06 확정)

**프로젝트 업데이트만 올린다.** 일지는 "어느 프로젝트를 어떻게 고쳤다"가 기본 단위다.

공부한 것·기술 메모(TIL)는 **옵시디언**에 쓴다. 사이트로 가져오지 않는다.
논의 문서(`docs/배경-논의정리.md`)에 나오는 '배운 것' 구역은 이 결정으로 만들지 않기로 했다.

그래서 새 일지에는 `tags` 에 프로젝트 이름을 적는 게 기본이다.
안 붙은 일지가 있으면 발행할 때 알려주지만, 막지는 않는다.
