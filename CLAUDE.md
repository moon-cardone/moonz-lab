...
사용자가 직접 올릴 때는 `publish.bat` 더블클릭 (안쪽은 `publish.py`).
같은 검사를 거치므로 잘못된 파일은 여기서도 올라가지 않는다.
`publish.bat` 에 한글을 넣지 말 것 — cmd 가 UTF-8 한글을 명령어로 잘못 읽는다.
사용자에게 보일 안내문은 전부 `publish.py` 에서 출력한다.
